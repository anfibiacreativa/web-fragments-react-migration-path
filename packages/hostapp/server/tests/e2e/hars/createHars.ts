...
test('Generate HAR for payment service and fragment request', async ({
  ...
  await paymentPage.goto('http://localhost:8080/store/catalogue');

  // simulate the payment request
  await paymentPage.evaluate(async () => {
    await fetch('http://localhost:8080/api/create-payment', {
      method: 'POST',
      body: JSON.stringify({
        amount: 49.99,
        currency: 'EUR',
  ...
  await fragmentPage.goto('http://localhost:8080/store/catalogue');

  // Simulate fragment asset request
  const response = await fragmentPage.request.get(
    'http://localhost:8080/_fragment/qwik/assets/build/some-file.js',
  );
